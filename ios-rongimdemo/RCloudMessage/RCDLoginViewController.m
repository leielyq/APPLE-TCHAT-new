...
- (void)viewDidLoad {
    ...
    [self.view addSubview:switchLanguage];
    ...
    self.countryTextField.hidden = true;
...
- (void)login:(NSString *)userName password:(NSString *)password {
    ...
    if ([self validateUserName:userName userPwd:password]) {
        ...
        NSString *s =  [self getMD5HashWithMessage:password];
        NSLog(@"%@", s);
        
        [[RCIM sharedRCIM] initWithAppKey:@"8brlm7uf8izu3"];
        [AFHttpTool loginWithPhone01:userName
            ...
            success:^(id response) {
                ...
                if ([response[@"flag"] intValue] == 1) {
                    ...
                    NSString *userId = response[@"data"][@"id"];
                    [self loginRongCloud:userName userId:userId token:token password:password];
                    [RCDSettingUserDefaults setUserToken:token];
                    [RCDSettingUserDefaults setuserDevice:self.currentRegion.phoneCode];
                    ...
                    switch (_errCode) {
                            ...
                            break;
                            case 2:
                            _errorMsgLb.text = RCDLocalizedString(@"错误次数超限，请半个小时后登陆");
                            break;
                            case 3:
                            _errorMsgLb.text = RCDLocalizedString(@"账号被封禁");
                            break;
                            case 4:
                            _errorMsgLb.text = RCDLocalizedString(@"账号或密码错误！");
                            break;
...
- (RCDIndicateTextField *)phoneTextField{
    if (!_phoneTextField) {
        ...
        _phoneTextField.indicateInfoLabel.text = RCDLocalizedString(@"phone");
