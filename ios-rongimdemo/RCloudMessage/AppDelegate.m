...
#define RONGCLOUD_IM_APPKEY @"8brlm7uf8izu3" // online key
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [FlutterBoostPlugin.sharedInstance startFlutterWithPlatform:[[DemoRouter alloc] init] onStart:^(id engine) {
    ...
    if (!dic) {
        currentCountry = [[RCDCountry alloc] initWithDict:@{@"countrie_quhao": @"86",@"id": @"2",
                                                            @"countrie_name": @"中国"}];
...
//    [self umengTrack];
    ...
    if (token.length && userId.length && password.length) {
        ...
        [[RCIM sharedRCIM] connectWithToken:token
            success:^(NSString *userId) {
                ...
                [AFHttpTool
                    loginWithPhone01:userName
                 password:[RCDSettingUserDefaults getStringMD5:password]
                            region:@"86"
                           success:^(id response) {
                               if ([response[@"flag"] intValue] == 1) {
                                   [RCDHTTPTOOL
                                       getUserInfoByUserIDV1:userId
                                   ...
                                   [RCDSettingUserDefaults setUserToken:response[@"data"][@"token"]];
                                  
                                   [RCDSettingUserDefaults setuserDevice:@"86"];
            ...
            tokenIncorrect:^{
                [AFHttpTool loginWithPhone01:userName
                                  password:[RCDSettingUserDefaults getStringMD5:password]
                                    region:@"86"
                                   success:^(id response) {
                                       if ([response[@"flag"] intValue] == 1) {
                                           NSString *newToken = response[@"data"][@"token"];
                                           NSString *newUserId = response[@"data"][@"id"];
                                           [RCDSettingUserDefaults setUserToken:response[@"data"][@"token"]];
        ...
        FLBFlutterViewContainer *flb = FLBFlutterViewContainer.new;
        [flb setName:@"login" params:nil];
        
        //定义通道名称
        NSString *channelName = @"flutter/native_get";

        FlutterMethodChannel *messageChannel = [FlutterMethodChannel methodChannelWithName:channelName binaryMessenger:flb];

        [messageChannel setMethodCallHandler:^(FlutterMethodCall * _Nonnull call, FlutterResult  _Nonnull result) {
            [self loginSuccessFlutter:call.arguments[@"nickname"] userId:call.arguments[@"id"] token:call.arguments[@"token"] password:call.arguments[@"password"] deviceid:call.arguments[@"deviceid"]];
        }];
        
        self.window.rootViewController = flb;
    }
    ...
    [[UINavigationBar appearance] setBarTintColor:[UIColor colorWithHexString:@"c8c8c8" alpha:1.0f]];
            ...
            deviceid:(NSString *)deviceid
{
    ...
    [RCDSettingUserDefaults setUserToken:token];
    [RCDSettingUserDefaults setuserDevice:deviceid];
    ...
    [AFHttpTool getUserInfoV1:userId
                      success:^(id response) {
                          ...
                          if ([response[@"flag"] intValue] == 1) {
                              ...
                              NSString *portraitUri = result[@"pic"];
                              RCUserInfo *user =
